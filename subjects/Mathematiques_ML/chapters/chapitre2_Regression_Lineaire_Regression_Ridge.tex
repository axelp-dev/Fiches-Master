...
\begin{equation}
    (H_0) : f_\theta = \varphi(x)^t \theta, \quad \theta \in \R^d.
\end{equation} 
...
\begin{proposition}
    On cherche à calculer explicitement $\hat{\theta} \in \arg \min_{\theta \in \R^d} \hat{R}(\theta)$. Utilisons la condition d'optimalité d'ordre 1 :
    \begin{equation}
...
On introduit la matrice de covariance empirique $\hat{\Sigma} := \frac{1}{n} \Phi^\top \Phi \in \R^{d \times d}$, associée à la semi-norme $\|u\|_{\hat{\Sigma}}^2 := u^\top \hat{\Sigma} u = \frac{1}{n} \|\Phi u\|_2^2$.
