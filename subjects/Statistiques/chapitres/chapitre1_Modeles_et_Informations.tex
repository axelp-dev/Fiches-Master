...
\begin{remark}[Double interprétation de la vraisemblance]
    La fonction $L(\theta; x)$ est un pivot entre deux mondes :
    \begin{itemize}
        \item A \emph{priori} (Modélisation) : Pour $\theta$ fixé, $x \mapsto L(\theta; x)$ est la \emph{densité} de l'échantillon. 
        Elle décrit la distribution des observations possibles.
        \item A \emph{posteriori} (Inférence) : Pour $x$ observé et fixé, $\theta \mapsto L(\theta; x)$ est la 
        \emph{vraisemblance}. Elle mesure la \emph{plausibilité} de chaque paramètre au vu des données. 
    \end{itemize}
    \textbf{Attention :} La vraisemblance n'est pas une mesure de probabilité sur $\Theta$. Elle permet de comparer deux 
    paramètres entre eux via le rapport $\frac{L(\theta_1; x)}{L(\theta_0; x)}$, mais sa valeur absolue n'a pas de sens 
    probabiliste intrinsèque.
\end{remark}
...
Voyons à présent la vraissemblance comme fonction de $ \theta$. Elle traduit alors la plausibilité 
que $ \theta$ soit le paramètre inconnu vis-à-vis de l'échantillon observé. On souhaiterait donc en déterminer 
le maximum pour trouver le $ \theta$ "le plus probable". Pour cela, on utilise la méthode 
du \emph{maximum de vraissemblance}. 

\begin{proposition}
    Soit $X = (X_1, \dots, X_n)$ un $n$-échantillon d'une variable aléatoire de loi $ \myP_{\theta_*}$ et 
    de densité $ f_\theta$. 
    On souhaite déterminer le paramètre $ \theta$, pour cela, on considère la vraissemblance de l'échantillon 
    définie par : 
        $$ \forall \theta \in \Theta, \quad L( \theta, X_1, \dots, X_n) = \prod_{i=1}^{n} f_\theta(X_i)$$
    dont on cherche le maximum. Or le produit n'est pas pratique à utiliser, notamment lors du calcul de la 
    dérivée. On utilise donc la fonction $ \log$. Par sa croissance, la $\log$-vraissemblance atteindra le 
    même maximum de la vraissemblance. On calcule donc : 
        $$ 
            \ell_n(\theta) = \log \left( \prod_{i=1}^{n} f_\theta(X_i) \right) 
            = \sum_{i=1}^{n} \log f_\theta(X_i).
        $$ 
    Après quelques vérifications, on calculera sa dérivée à annuler pour trouver une valeur de 
    $ \theta$ candidate, noté $ \hat{\theta}_n$, que l'on conservera après avoir confirmé la concavité de $ \ell_n$ en étudiant 
    le signe de sa dérivée seconde. 
    Le $ \hat{\theta}_n$ sera appelé un \emph{estimateur} et sera une fonction de $(X_1, \dots, X_n)$. 
\end{proposition}

\begin{definition}[Estimateur]
    Dans un modèle $( \mathcal{X}, \mathcal{A}, \myP_\theta)$ dominé par $ \mu$, un \emph{estimateur du maximum de 
    vraissemblance} est une statistique $ \hat{\theta}$ telle que : 
        \begin{equation}
            L( \hat{\theta}, X_1, \dots, X_n) = \max_{\theta \in \Theta} L(\theta, X_1, \dots, X_n)
        \end{equation} 
    pour n'importe quel échantillon $(X_1, \dots, X_n)$ de loi $ \myP_{\theta_*}$. 
\end{definition}
...
Cette notion est fondamentale pour isoler le "bruit" du "signal" ($\theta$). Le lien avec l'exhaustivité est donné par 
le théorème suivant, très utile pour montrer l'indépendance de deux variables ou pour prouver qu'une statistique n'est pas exhaustive.

\begin{theorem}[Théorème de Basu]
    Si $T$ est une statistique \emph{exhaustive complète} et $S$ est une statistique \emph{libre}, alors $T$ et $S$ 
    sont \emph{indépendantes} sous toute loi $\mathbb{P}_\theta$.
\end{theorem}

\begin{example}
    Dans un modèle gaussien $\mathcal{N}(\mu, \sigma^2)$ avec $\sigma^2$ connu, la moyenne empirique $\bar{X}_n$ est 
    exhaustive complète. La variance empirique $S_n^2$ (ou toute valeur centrée $X_i - \bar{X}_n$) est libre. 
    Par le théorème de Basu, $\bar{X}_n$ et $S_n^2$ sont indépendantes.
\end{example}
...
\section{Informations Qualitatives}
...
\begin{definition}[Modèle Exponentiel]
    ...
    Il sera qualifié \emph{de plein rang} si $p=q$. On dira aussi qu'il est \emph{canonique} si $ p=q$ et 
...
\begin{prop}[Statistique privilégiée]
    Dans le cas d'un modèle exponentiel, la statistique $T$ est \emph{exhaustive, complète} et 
    \emph{minimale}.
\end{prop}
...
\begin{definition}[Information de Fisher]
    ...
    \begin{equation}
        \boxed{\mathcal{I}_\theta(X) = \mathbb{E}_\theta(S_\theta(X) \times S_\theta^t(X))}
    \end{equation}
...
\begin{prop}[Cas du modèle exponentiel canonique]
    Dans le cas d'un modèle exponentiel canonique, on a : 
        $$ \mathcal{I}_\theta(X) = \V_\theta(T(X)) = - H_{ \kappa_\theta} $$
    où $ H_{ \kappa_\theta}$ est la hessienne de $ \kappa_\theta = \log k_\theta$. 
