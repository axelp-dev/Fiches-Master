...
\begin{definition}[Transformée de Fourrier]
    ...
    aussi périodique de période $1$, i.e $X(f + 1) = X(f)$.
...
% On introduit ici la notion de \emph{filtre sélectif}. On cherche ici à traiter un signal pour en 
% extraire certaines informations telles qu'un certain domaine de fréquence, l'énergie d'un signal 
% ou sa puissance. 

% \begin{definition}[Filtre]
%     Un filtre est un système linéaire invariant par translation (SLIT) de réponse 
%     impultionnelle $h$. On appelle sa transformée de Fourrier $H(f)$ sa \emph{fonction de transfert}. 
%     Un \emph{filtre sélectif} est un filtre destiné à isoler/conserver certaines fréquences du signal.     
% \end{definition}

% \begin{example}[Filtres]
%     Voyons à présent quelques exemples de filtres permettant de conserver/supprimer 
%     les fréquences hautes ou basses : 
...
% \begin{tikzpicture}
%     \begin{axis}[
%         axis lines = middle, % Centre les axes sur l'origine
%         xlabel = {$f$},
%         ylabel = {$|H(f)|$},
%         % Définit les limites visibles explicitly
%         xmin = -2.5, xmax = 2.5,
%         ymin = -0.1, ymax = 1.3,
%         % Configure les graduations
%         xtick = {-1, 1},
%         xticklabels = {$-f_c$, $f_c$},
%         ytick = {1},
%         yticklabels = {1},
%         % Supprime la grille pour un rendu épuré
%         grid = none,
%         % Taille du graphique
%         width = 10cm,
%         height = 6cm,
%         % Titre (optionnel)
%         title = {Filtre Passe-Bas Idéal Symétrique},
%         scale=0.7
%     ]

%     % Tracé du filtre idéal (indicatrice) en bleu, très épais
%     \addplot[blue, thick, no marks] coordinates {
%         % Bande d'arrêt négative
%         (-2.5, 0)
%         (-1.0, 0)
%         % Transition verticale gauche
%         (-1.0, 1.0)
%         % Bande passante
%         (1.0, 1.0)
%         % Transition verticale droite
%         (1.0, 0)
%         % Bande d'arrêt positive
%         (2.5, 0)
%     };

%     % Optionnel : Griser légèrement la zone de la bande passante pour la faire ressortir
%     % \fill[blue, opacity=0.1] (axis cs:-1,0) rectangle (axis cs:1,1);
...
%         height = 6cm,
%         % Titre (optionnel)
%         title = {Filtre Passe-Haut Idéal Symétrique},
%         scale=0.7
%     ]
...
%     Et des filtres permettant de sélectionner/supprimer un intervalle de fréquences :
...
%         % Configure les graduations
%         xtick = {-1.5, -1, 1, 1.5},
%         xticklabels = {$-f_{c_2}$, $-f_{c_1}$, $f_{c_1}$, $f_{c_1}$},
%         ytick = {1},
%         yticklabels = {1},
%         % Supprime la grille pour un rendu épuré
%         grid = none,
